        ...
        <header class="py-4">
            <div class="flex flex-col gap-2 pt-4 md:pt-6">
                ...
                <div id="article-title" class="leading-snug">
                    <p class="text-3xl font-bold text-[var(--c-100)] mb-4">
                        {{ post.title }}
                    </p>
                </div>
...
watch(
    ...
    async (newId) => {
        ...
        const currentPost =
            postStore.getPostById(newId as string) ?? createEmptyPost();
        post.value = currentPost;

        const index = postStore.posts.findIndex((p) => p.id === newId);
        prevPost.value = index > 0 ? postStore.posts[index - 1] : null;
        nextPost.value =
            index >= 0 && index < postStore.posts.length - 1
                ? postStore.posts[index + 1]
                : null;
    }
);
...
onMounted(async () => {
    ...
    if (props.isPage) {
        post.value =
            postStore.getPostById(props.pageId as string) ?? createEmptyPost();
    } else {
